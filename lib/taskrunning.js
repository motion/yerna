...
const { linkPackages } = require('./linking');
...
function prelink(packagesByName) {
  ...
  console.error(`yerna: linking all ${_.size(packagesByName).toString().cyan} package(s) before running tasks`);
}

function postlink(packagesByName) {
  linkPackages(packagesByName);
  console.error(`yerna: re-linking all ${_.size(packagesByName).toString().cyan} package(s) after running tasks`);
}
